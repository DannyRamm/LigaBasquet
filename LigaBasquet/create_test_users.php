...
foreach ($usuarios as $usuario) {
    ...
    $stmtCheck = $conector->prepare('SELECT codUsu FROM Usuario WHERE corUsu = ? LIMIT 1');
    $stmtCheck->bind_param('s', $usuario['corUsu']);
    $stmtCheck->execute();
    $usuarioExistente = $stmtCheck->get_result()->fetch_assoc();
    
    if ($usuarioExistente) {
        // Si existe, actualizar contraseña y rol
        $stmt = $conector->prepare('UPDATE Usuario SET pasUsu = ?, codRol = ? WHERE corUsu = ?');
        $stmt->bind_param('sis', $usuario['pasUsu'], $codRol, $usuario['corUsu']);
        $stmt->execute();
    } else {
        // Si no existe, insertar
        $stmt = $conector->prepare('INSERT INTO Usuario (nomUsu, corUsu, pasUsu, codRol, fecRegUsu) VALUES (?, ?, ?, ?, NOW())');
        $stmt->bind_param('sssi', $usuario['nomUsu'], $usuario['corUsu'], $usuario['pasUsu'], $codRol);
        $stmt->execute();
    }
}

echo "Usuarios de prueba creados/actualizados exitosamente.<br>";
